...
### Android 登录出现 `Failed to fetch`

Capacitor Android 的请求来源为 `https://localhost`。原生登录 POST 会携带 `Content-Type`、`X-HL6-Client-Key` 和 `X-Idempotency-Key`，部署服务端后必须确认预检响应包含：
...
若缺失该响应头，通常是运行中的容器仍是旧镜像，或反向代理移除了 CORS 响应头。更新服务端镜像后，按照 [Android 客户端文档](native-client.md#cors-deployment-check) 的 `OPTIONS` 检查命令复验；无需向 OIDC 提供商添加 Android 深链。
