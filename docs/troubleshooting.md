...
配置 `/etc/docker/daemon.json`（Linux）或 Docker Desktop 设置：
...
> 镜像加速器地址经常变动，如以上不可用请搜索"Docker 镜像加速器 2026"获取最新地址。

### 3.3 私有镜像仓库

HL6 镜像地址为 `ghcr.milu.moe/hanlull/hl6:latest`。如该地址不可达，可：
...
`postgres:16-alpine` 拉取失败时，可改用其他镜像源或手动拉取后重打标签：
...
| 商户密钥错误 | 签名校验失败，回调会被拒绝；核对 `EPAY_KEY` / `CODEPAY_KEY` |
