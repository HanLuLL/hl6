...
| 2026-07-21 | 合并管理后台日志页：`/admin/audit-logs`、`/admin/email-logs`、`/admin/logs` 统一收敛到 `/admin/logs` 单一水平 Tab 页（审计/邮件/系统三 Tab），旧路由通过 `<Navigate>` 重定向到带 `?tab=` 参数的新路径。Tab 列表在 Android 窄屏使用 `overflow-x-auto` 水平滚动，无需原生改动 |
| 2026-07-21 | 合并内容审查页：`/admin/ai-audit` 合入 `/admin/audit`，新增 `ai-models`/`ai-reviews`/`ai-templates` 三个 Tab，`/admin/ai-audit` 重定向到 `/admin/audit?tab=ai-models`。AI 审查采用串联式接入：规则先判定，未命中且 clean 时再触发 AI 复核。所有 Tab 在 Android 窄屏使用水平滚动 TabsList，无原生改动 |
| 2026-07-21 | 修复被封禁用户申诉页面：申诉表单在 Android 窄屏使用 `max-w-lg` 卡片 + 垂直堆叠布局，提交按钮全宽，封禁原因/截止时间/申诉历史块级排列，无需原生改动 |
| 2026-07-21 | 新增实名认证系统（共享 Web/Android UI）：头像下拉菜单新增「账户安全」入口（在登出之上），包含邮箱绑定信息卡 + 实名认证表单（姓名/身份证号/验证方式/支付方式）。后端新增 `RealnameApplication` 模型（姓名/身份证 AES-256-GCM 加密存储，API 响应脱敏展示），`PaymentOrder.Type` 字段分发 credits/realname 两种业务类型，支付回调事务内更新状态后 goroutine 异步调用第三方 API。后台「系统设置-实名认证」Tab 配置启用开关、认领/充值强制实名、费用、服务商（manual/aliyun/juhe）、AppCode/AppKey（加密存储）、人脸开关。管理员侧栏新增「实名审核」入口（`/admin/realname`），支持按状态/服务商/用户 ID 筛选、人工审核通过/拒绝、失败重试。所有页面在 Android 窄屏使用 `max-w-lg`/卡片堆叠/水平滚动 TabsList 布局，无原生改动 |
