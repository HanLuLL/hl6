...
6. 服务端必须允许 Capacitor 来源 `https://localhost`，并在接受来源时返回 `Vary: Origin`。CORS 允许不代表绕过通讯密钥或会话校验。
...
5. 常规构建工作流不得自动创建 GitHub Release、发布 APK 或上传 APK。任何外部交付都需要仓库所有者明确授权并配置专用渠道。
6. GitHub Actions Artifact 下载固定为 ZIP，禁止将其描述为直接 APK 下载。需要直接 APK 时，必须另行选择获批的 Release 或可信 HTTPS 分发地址。
...
4. 更新链接必须指向经明确批准、可信的 HTTPS APK 分发地址，不默认指向 GitHub Release。
