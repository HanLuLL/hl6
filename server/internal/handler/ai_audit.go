...
// AIAuditHandler AI 审查相关 API handler。
type AIAuditHandler struct {
	repo          *repository.Repository
	encryptionKey []byte
}

// NewAIAuditHandler 创建 AI 审查 handler。
...
func (h *AIAuditHandler) ListModelConfigs(c *gin.Context) {
	...
	type modelConfigView struct {
		model.AIModelConfig
		APIKey string `json:"api_key"` // 覆盖原字段，脱敏
	}
...
// ReviewAIReview 管理员二次审核 AI 审查记录。
func (h *AIAuditHandler) ReviewAIReview(c *gin.Context) {
	...
	var body struct {
		Status string `json:"status" binding:"required"` // confirmed / overturned / dismissed
