...
export function RecordTable({ subdomainId, records }: RecordTableProps) {
  ...
  const handleDelete = (record: DNSRecord) => {
    if (!confirm(`Delete ${record.type} record pointing to ${record.content}?`)) return;
  ...
  if (records.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <p className="text-xs text-muted-foreground mt-1">Add your first record to get started</p>
      </div>
    );
  }
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="w-20">Proxied</TableHead>
          ...
          <TableBody>
            {records.map((record) => (
              <TableRow key={record.id}>
                ...
                <TableCell>
                  {record.proxied ? (
                    <span className="text-orange-500 text-sm">On</span>
                  ) : (
