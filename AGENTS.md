...
>
> **容器镜像**：`ghcr.nju.edu.cn/hanlull/hl6:latest`（南大镜像站，无需登录直接拉取）

## Tech Stack

- **Frontend**: React 19 + TypeScript, Vite, Tailwind CSS 4, React Router DOM, TanStack React Query, i18nextt (6 语言), 邮箱密码认证, Shadcn UI
- **Backend**: Go (Gin + GORM), PostgreSQL 16, Cloudflare API (DNS 管理), JWT 认证 (lestrrat-go/jwx)
- **Infra**: Docker Compose (PostgreSQL), PostgreSQL 维护工具, Cloudflare (DNS)
...
- **认证流程**: 前端邮箱验证/密码登录 → JWT 会话 → 后端 middleware 验证；Android 原生会话同时验证通讯密钥
...
- **AI 内容审查**: 集成 OpenAI API 兼容的 LLM，支持模型配置管理、提示词模板、审查记录、自动处置、管理员二次审核；后台 `/admin/ai-audit` 管理
- **用户申诉系统**: 被封禁用户可在 `/banned` 页面查看封禁原因并提交申诉，管理员审批通过可自动解封
- **AI 模型配置**: API Key 加密存储（AES-256-GCM），脱敏展示，支持多模型、限流（RPM）、温度调节
- **审查提示词模板**: 内置通用/严格/代码安全三套默认模板，管理员可自定义，支持变量插值（`{{fqdn}}`、`{{content}}` 等）
